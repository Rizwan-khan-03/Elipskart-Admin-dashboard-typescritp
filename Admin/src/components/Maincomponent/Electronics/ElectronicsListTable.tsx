...
import { getElectronicsList } from '../../../App/Service/service.electronics';
...
const TableExample = () => {
  ...
  const handlGetProductList = async () => {
    try {
      ...
      const res: any = await dispatch(getElectronicsList({ userId: "6454fa649b0ffa5392ed86ba", isAdmin: true ,categories:'electronics'}))
  ...
  return (
    <>
      <Box sx={{ display: 'flex', justifyContent: "space-between", marginBottom: '10px' }}>
        <Title>Electronics Indent</Title>
        <Button variant='outlined' onClick={() => setOpen(true)}>Add Electronics</Button>
      </Box>
      ...
      <Modal
          content={<ElectronicsForm setOpen={setOpen} setTableData={setTableData} tableData={tableData} />}
          open={open}
          setOpen={setOpen}
