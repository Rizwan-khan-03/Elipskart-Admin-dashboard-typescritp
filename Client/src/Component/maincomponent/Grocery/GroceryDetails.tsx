...
export default function GroceryDetails({ groceryData }: any) {
                ...
                <Box >
                    <Grid container spacing={2}>
                        <Grid item xs={12} md={6}>
                            <Item>
                                <MobileName>{groceryData?.title?.toUpperCase()} {" "} 
                                </MobileName>
                                <Rating>
                                    <span>{groceryData?.ratings?.overallRating}*
                                    ...
                                    <p>{groceryData?.ratings?.totalRatings} Rating &amp; {groceryData?.ratings?.totalReviews}  Reviews</p>
                                </Rating>
                                <MobilePrice>
                                    <MobilePercent>
                                        <span>Extra</span>
                                        <span> <i className="fa fa-inr"></i>10 off</span>
                                    </MobilePercent>
                                </MobilePrice>
                                <MobileName> <span> <i className="fa fa-inr"></i> {" "}{groceryData?.price}</span></MobileName>

                                {/* <FeatureList>
                                    <FeatureItem> <span>{groceryData?.features?.ram}</span> | <span>{groceryData?.features?.rom}</span> | <span>Upto</span> | </FeatureItem>
                                    <FeatureItem> <span>{groceryData?.features?.screenSize}</span> </FeatureItem>
                                    <FeatureItem> <span>{groceryData?.features?.secondaryCamera}</span>| <span>{groceryData?.features?.primaryCamera}</span> </FeatureItem>
                                    <FeatureItem> <span> {groceryData?.features?.batteryCapacity}</span> </FeatureItem>
                                    <FeatureItem> <span> {groceryData?.features?.warranty}</span> </FeatureItem>
                                </FeatureList> */}
                            </Item>
                        </Grid>
